...
static const char* g_strPluginName = "GTOPO30 Header";
static const char* g_strMasks      = "*.hdr";
...
void getBadElevationColor( int layerID, unsigned char* pR,
                           unsigned char* pG, unsigned char* pB )
{
    RasterPluginLayer* pLayer = g_layers.getLayer( layerID );
    if ( NULL != pLayer )
    {
        pLayer->getBadElevationColor( *pR, *pG, *pB );
    }
}
...
bool loadOverlay( int layerID, const char* strFilename )
{
    ...
    RasterPluginLayer* pLayer = new GTOPO30Layer( layerID, strFilename, (struct HINSTANCE__ *)g_hInstance );
...
float getVerticalSpatialResolution( int layerID )
{
    RasterPluginLayer* pLayer = g_layers.getLayer( layerID );
    if ( NULL != pLayer )
    {
        return pLayer->getVerticalSpatialResolution();
    }
...
bool showBadElevations( int layerID )
{
    RasterPluginLayer* pLayer = g_layers.getLayer( layerID );
    if ( NULL != pLayer )
    {
        return pLayer->showBadElevations();
    }
