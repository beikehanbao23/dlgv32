...
public:
  // Construction
  RasterPluginLayer( int layerID, const char* strFilename, 
                     HINSTANCE hInstance );
  ...
  virtual bool getElevation( long row, long col, float& elev ) const;
    /* Retrieves the elevation at pixel coordinates (<row>,<col>).  Returns
       false if the coordinates are invalid. Override this method in vertical 
       layers */
  ...
  virtual void getMinMaxSampleValue( float& minElev, float& maxElev ) const;
    /* Retrieves the minimum and maximum elevation for this layer.  This only
       applies to vertical layers.  The default returns 0.0 for both 
       parameters */

  virtual bool getRowColColor( long row, long col, unsigned char& r,
                               unsigned char& g, unsigned char& b ) const;
    /* Retrieves the RGB color at pixel coordinates (<row>,<col>).  Override 
       this method in non-vertical layers.  Returns false if the coordinates
       are invalid */
  ...
  virtual void getBoundingRect( double& left, double& bottom, double& right, 
                                double& top ) const = 0;
  ...
  virtual void getProjection( PROJSYS& projSys, DATUM& datum, UNIT& unit,
                              int& zone, double* pParams ) const = 0;
...
inline bool
RasterPluginLayer::getRowColColor( long row, long col, unsigned char& r, 
                                   unsigned char& g, unsigned char& b ) const
{
  return false;
}
