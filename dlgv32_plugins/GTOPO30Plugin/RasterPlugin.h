// RasterPluginTest.h - interface for RasterPlugin DLL exports
//
...
__declspec(dllexport) void getBadElevationColor( int layerID, unsigned char* pR,
                                                 unsigned char* pG, unsigned char* pB );
__declspec(dllexport) void getBoundingRect( int layerID, double* pLeft, double* pBottom,
                                          double* pRight, double* pTop );
__declspec(dllexport) const char* getDescription( int layerID );
__declspec(dllexport) bool getElevation( int layerID, long row, long col, float* pElev );
__declspec(dllexport) long getImageHeight( int layerID );
__declspec(dllexport) long getImageWidth( int layerID );
__declspec(dllexport) void getMinMaxSampleValue( int layerID, float* pMin, float* pMax );
__declspec(dllexport) void getProjection( int layerID, PROJSYS* pProjSys, DATUM* pDatum,
                                        UNIT* pUnit, int* pZone, double* pParams );
...
__declspec(dllexport) float getVerticalSpatialResolution( int layerID );
